...
api.process = function process (data, callback) {
  ...
  screenStream.on('warn', function onWarn (err) {
    reqLog.warn({
      err: err
    }, 'screenStream warn');
  });
  screenStream.on('warning', function onWarn (err) {
    reqLog.warn({
      err: err
    }, 'screenStream warning');
  });
...
function upload (fileName, inputStream, reqLog, callback) {
  ...
  if (config.local) {
    uploadStream = fs.createWriteStream(path.join(config.filepath, fileName));
    uploadURL = path.relative(basePath, config.filepath) + '/' + encodeURIComponent(fileName);
  } else {
    uploadStream = gfs.uploadStream(fileName);
    uploadURL = gfs.uploadUrl(fileName);
  }

  pump(inputStream, uploadStream, function onPumpError (err) {
    if (err) {
      reqLog.error({
        err: err,
        fileName: fileName
